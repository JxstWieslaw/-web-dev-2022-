...
const xrComponents = () => {
  ...
  const loadingComponent = {
    ...
    init() {
      ...
      window.XRExtras
        ? load()
        : window.addEventListener("xrextrasloaded", load, { once: true });
      ...
      const requestingCameraPermissions = document.querySelector(
        "#requestingCameraPermissions"
      );

      if (requestingCameraPermissions) {
        requestingCameraPermissions.style.backgroundColor =
          this.data.cameraBackgroundColor;
      }
    },
    remove() {
  ...
  const statsComponent = {
    ...
    init() {
      ...
      if (window.XRExtras && window.XR8) {
        ...
        window.addEventListener("xrandextrasloaded", this.loadModule, {
          once: true,
        });
      }
    },
    ...
    init() {
      ...
      const addComponents = ({ detail }) => {
        detail.imageTargets.forEach(({ name, type, metadata, properties }) => {
          ...
          el.setAttribute(
            "rotated",
            properties && properties.isRotated ? "true" : "false"
          );
          el.setAttribute(
            "metadata",
      ...
      if (this.internalState.dragging) {
        ...
        if (this.internalState.positionRaw) {
          const screenPositionX =
            (this.internalState.positionRaw.x / document.body.clientWidth) * 2 -
            1;
          const screenPositionY =
            (this.internalState.positionRaw.y / document.body.clientHeight) *
              2 -
            1;
    ...
    init() {
      const load = () => {
        ...
        if (delayAfterDismissalMillis >= 0) {
          config.promptConfig.delayAfterDismissalMillis =
            delayAfterDismissalMillis;
        }
        if (minNumVisits >= 0) {
        ...
        if (
          Object.keys(config.promptConfig).length ||
          Object.keys(config.displayConfig).length
        ) {
    ...
    init() {
      ...
      const beforeRun = ({ detail }) => {
        ...
        if (this.el.getAttribute("material")) {
          ...
          material = this.el.sceneEl.querySelector(
            this.data["material-resource"]
          ).material;
        } else {
  ...
  const captureConfigComponent = {
    ...
    update() {
      ...
      if (this.attrValue.excludeSceneAudio !== undefined) {
        console.warn(
          '"exclude-scene-audio" has been deprecated in favor of "include-scene-audio"'
        );
        config.configureAudioOutput = this.data.excludeSceneAudio
          ? null
          : this.includeSceneAudio;
      } else {
        config.configureAudioOutput = this.data.includeSceneAudio
          ? this.includeSceneAudio
          : null;
      }
  ...
  const targetMeshComponent = {
    ...
    init() {
      ...
      const geometry = ["full", "label"].includes(this.data.geometry)
        ? this.data.geometry
        : "label";
  ...
  const curvedTargetContainerComponent = {
    ...
    init() {
      ...
      const createCurvedContainerGeometry = (
        geometry,
        isFull,
        userHeight,
        userWidth
        ...
        const length =
          (2 * Math.PI - geometry.arcLengthRadians) * (userWidth || 1);
        const open = isFull;
      ...
      const constructGeometry = ({ detail }) => {
        ...
        const openingGeo = createCurvedContainerGeometry(
          detail,
          false,
          userHeight,
          userWidth
        ...
        const topGeo = createCurvedContainerGeometry(
          detail,
          true,
          userHeight,
          userWidth
        ...
        const bottomGeo = createCurvedContainerGeometry(
          detail,
          true,
          userHeight,
          userWidth
        ...
        const intBackMat = new THREE.MeshStandardMaterial({
          color: intColor,
          side: THREE.BackSide,
        });
        const intFrontMat = new THREE.MeshStandardMaterial({
          color: intColor,
        ...
        const intGeo = createCurvedContainerGeometry(
          detail,
          true,
          userHeight,
          userWidth
