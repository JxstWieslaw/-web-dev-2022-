...
var distZ = 30;
var friction = 0.01;
myObject.style.webkitTransform = 'rotateY(' + distX + 'deg) rotateX(' + distY + 'deg) rotateZ(' + distZ + 'deg)';
...
mc.on("panleft panright panup pandown", function(ev) {
  ...
  if (ev.type == 'panleft' || ev.type == 'panright') {
    distX = parseFloat(myObject.style.webkitTransform.split(' ')[0].split(/[()]/)[1]) + (ev.deltaX * friction);
  }
  if (ev.type == 'panup' || ev.type == 'pandown') {
    distY = parseFloat(myObject.style.webkitTransform.split(' ')[1].split(/[()]/)[1]) + (ev.deltaY * friction);
  }
...
mc.on("rotatestart rotateend rotatemove", function(ev) {
	var rotation = parseFloat(myObject.style.webkitTransform.split(' ')[2].split(/[()]/)[1]) + (ev.rotation/100);
	distZ =  parseFloat(myObject.style.webkitTransform.split(' ')[2].split(/[()]/)[1]) - rotation;
  myObject.style.webkitTransform = 'rotateY(' + distX + 'deg) rotateX(' + distY + 'deg) rotateZ(' + distZ + 'deg)';
});
mc.on("rotateend", function(ev) {
	currentRotation = parseFloat(myObject.style.webkitTransform.split(' ')[2].split(/[()]/)[1]);
});